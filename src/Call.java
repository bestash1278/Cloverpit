...
public class Call {
    ...
    // 생성자 (필요한 로직 객체 주입)
    public Call(User user, RoundManager roundManager) {
        this.user = user;
 ...
 // ⭐ 새로 추가: 리롤만 할 때 티켓을 차감하는 함수 (핵심)
    public boolean useCallForReroll() {
    	int Reroll_cost = user.getCallReroll_count() * 2;
    ...
    public boolean useCall(int selectionIndex) {
        if (user.getCall_count() > 0 && selectionIndex >= 0 && selectionIndex < currentSelections.size()) {
